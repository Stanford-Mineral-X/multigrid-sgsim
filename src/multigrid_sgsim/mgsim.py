...
def mgsim(
    ...
    for (i,mg_resol) in enumerate(mg_resols):
        ...
        # mgsgs is positionally aligned with df: pred_xy_grid was built from df's
        # rows in order, and gstatsim returns values in prediction_grid order.
        assert len(mgsgs) == len(df)
...
def mgsim_nst(
    ...
    for (i, mg_resol) in enumerate(mg_resols):
        ...
        # update trend on grid and obspts (trend = trend + simulated_residuals)
        # mgs_invtrans is positionally aligned with df: pred_xy_grid was built from
        # df's rows in order, and gstatsim returns values in prediction_grid order.
        assert len(mgs_invtrans) == len(df)
