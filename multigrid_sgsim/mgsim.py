...
def mgsim(
    ...
    """
    Perform multigrid sequential Gaussian simulation (MGSIM) to update the
    residual field and trend of a dataset.

    If ``nst_trans`` is provided, simulation runs in normal-score space: the
    residuals are transformed with the fitted transformer, simulated, and the
    simulated values are back-transformed (optionally clipped to the observed
    range) before being added to the trend. Without ``nst_trans``, residuals
    are simulated directly in their original units.
    ...
    df_xyvtcs : pd.DataFrame
        ...
        - 'cluster' (integer cluster ID for each data point; -1 for locations not to be simulated)
    ...
    xx, yy, zz, kk : str
        Column names in df_xyvtcs for x, y, conditioning value, and cluster ID.
        ``zz`` defaults to 'Nresidual' when ``nst_trans`` is given, else 'residual'.
    num_points : int
        Number of nearest neighbors to use in SGS step
    radius : float
        Search radius for nearest neighbors in SGS step
    sgs_or_krige : str
        'sgs' for stochastic simulation, 'krige' for ordinary kriging updates
    nst_trans : QuantileTransformer, optional
        Fitted normal-score transformer for the residuals. Enables NST mode.
    clip_nst : bool
        NST mode only. If True (default), clip simulated values to the observed
        range in normal space before inverse transforming, and again in original
        space afterwards, to prevent residual drift.
    ...
    --------
    df_all : pd.DataFrame
        DataFrame with updated 'newtrend' column after multigrid simulation;
        retains all original rows and indices. 'newtrend' (and 'residual', plus
        'Nresidual' in NST mode) is NaN for rows where 'cluster' < 0 (not simulated).
    ...
    for (i, mg_resol) in enumerate(mg_resols):
        ...
        if use_nst:
            ...
            # inverse transform back to original residual units
            sim_residuals = nst_trans.inverse_transform(data2inorm).ravel()

            # also clip in original space to prevent residual drift
            if clip_nst:
                sim_residuals = np.clip(sim_residuals, res_min, res_max)

            if debug:
                print(f"  After inverse transform (clipped): min={sim_residuals.min():.2f}, "
                      f"max={sim_residuals.max():.2f}, mean={sim_residuals.mean():.2f}, "
                      f"std={sim_residuals.std():.2f}")
        else:
            sim_residuals = mgsgs

        # update trend on grid and obspts (trend = trend + simulated_residuals)
        # sim_residuals is positionally aligned with df: pred_xy_grid was built from
        # df's rows in order, and gstatsim returns values in prediction_grid order.
        assert len(sim_residuals) == len(df)
        df['newtrend'] = df['newtrend'] + sim_residuals
    ...
    # stitch back to full frame (columns are NaN where cluster < 0)
