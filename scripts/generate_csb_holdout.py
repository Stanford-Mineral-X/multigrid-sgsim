#!/usr/bin/env python
"""
Generate CSB holdout dataset by withholding 3 N-S flight lines.

Produces:
  - csb_xyvmmc_3linesout.csv : same format as original, fl_mask=0 for withheld points
  - csb_holdout_3lines.csv   : just the withheld observations (for validation)
  - csb_holdout_3lines.json  : metadata (x values, tolerance, counts)

Mirrors the holdout logic from sandbox/csb_work.ipynb.

Usage:
    python generate_csb_holdout.py
"""
...
HOLDOUT_X_VALUES = [549000, 553650, 555250]
TOL = 50  # tolerance in coordinate units


def main():
    print("=" * 60)
    print("CSB HOLDOUT DATASET GENERATOR")
    print("=" * 60)
    ...
    holdout_path = DATA_DIR / 'csb_holdout_3lines.csv'
    ...
    reduced_path = DATA_DIR / 'csb_xyvmmc_3linesout.csv'
    ...
    metadata = {
        ...
        'per_line': per_line,
        'holdout_file': 'csb_holdout_3lines.csv',
        'reduced_file': 'csb_xyvmmc_3linesout.csv',
    }

    meta_path = DATA_DIR / 'csb_holdout_3lines.json'
